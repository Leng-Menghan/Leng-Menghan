<h1 align="center">👋 Hi, I'm Leng Menghan</h1>
<br>
<h1 align="center">🎯 Mobile and Web Developer Intern | Software Engineering Student at CADT</h1>
<br>
<hr style="border:0.2px solid #ddd;">

## 🚀 About Me

I am a Software Engineering student at the Cambodia Academy of Digital Technology (CADT) with a strong passion for mobile and web development.

I enjoy solving real-world problems, building clean user interfaces, and developing full-stack applications. I work well both independently and in teams, and I always aim to deliver high-quality results on time.

---

## 🛠️ Tech Stack

* **Mobile Development:** Flutter, Dart
* **Frontend:** React.js, HTML, CSS, JavaScript, jQuery, Bootstrap
* **Backend:** Node.js, Express.js, PHP
* **Database:** MySQL, SQLite, Firebase
* **Programming Languages:** C, C++, Java

---

## 📱 Featured Projects

### 🔹 Tos Review App (Capstone Project)

* Built with **Flutter + Next.js + Prisma**
* Features: Review posts, ratings, comments, notifications, user profiles
* Role: Frontend Developer (Flutter)

🔗 GitHub: https://github.com/pychey/Tos-Review-App.git
🎥 Demo: https://youtu.be/NXkKeY3T8n4

---

### 🔹 Financial Tracking App

* Built with **Flutter & SQLite**
* Features: Expense & Income tracking, statistic summary, set Budget goal and user profile
* Supports **Khmer & English UI**

🔗 GitHub: https://github.com/Leng-Menghan/fundamental_flutter_project.git
🎥 Demo: https://youtube.com/shorts/rRYcxHTXf2E

---

### 🔹 POS Management System

* Built with **React + Node.js API**
* Features: Order management, dashboard, staff management, table management and menu management
* Role: Team Leader (Backend + Frontend Integration)

🔗 GitHub: https://github.com/Leng-Menghan/Pos_Management_System.git
🎥 Demo: https://youtu.be/8_SCeUhNP8o

---

## 💼 Other Projects

* HR Management System (PHP & MySQL)
* School Management API (Express.js + MySQL + JWT)
* Library Management System (Java)
* Hotel Reservation System (MySQL)
* E-commerce Website (HTML & CSS)
* Shopping Cart System (jQuery)

---

## 🎓 Education

* **Cambodia Academy of Digital Technology (CADT)**
  🎓 Bachelor of Computer Science (Software Engineering)
  GPA: 3.83 (Scholarship Student)

* **CamTech University**
  📘 English Level: B2 (Upper-Intermediate)

---

## 🏆 Achievements

* 🥈 Grade 12 - National Physics Outstanding Student (Rank 8)
* 🥇 Grade 12 – Kandal Province Physics Outstanding Student (Rank 1)
* 🥇 Grade 9 – Kandal Province Physics Outstanding Student (Rank 1)

---

## 🌐 Languages

* Khmer (Native)
* English (Intermediate - B2)

---

## 📫 Contact Me

* 📧 Email: [han230666@gmail.com](mailto:han230666@gmail.com)
* 📱 Phone: +855 16 556 528
* 💬 Telegram: https://t.me/Leng_MengHAN
* 💻 GitHub: https://github.com/Leng-Menghan

---

⭐ *I'm currently looking for a Mobile Developer Internship opportunity. Feel free to connect with me!*
